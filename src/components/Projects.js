...
const Projects = () => {
  const [selectedProject, setSelectedProject] = useState(null); // Track which project is expanded
  ...
  const handleClose = () => {
    setSelectedProject(null); // Close the expanded view
  };

  return (
    <section id= "projects" style={{ backgroundColor: '#121212', color: '#fff', padding: '50px', textAlign: 'center' }}>
      <h2>My Projects</h2>
      <div className="projects-container">
        <div className="project-card animate__animated animate__fadeInUp" onClick={() => handleExpand(1)}>
          <img src={recipeRadarImg} alt="Recipe Radar Screenshot" />
          <h3>Recipe Radar Webapp</h3>
          <h4>HackTheNorth 2024 Submission</h4>
          <p>blah blah blah preview</p>
          <a href="https://github.com/shawny4/HTN-RecipeFinder" className="button">View Repo</a>
        </div>
        {/* expanded view for recipe radar */}
        {selectedProject === 1 && (
